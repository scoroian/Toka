...
- Create: `functions/test/integration/dispatch_due_reminders.test.ts`
...
## Task 9: dispatch_due_reminders.test.ts (scheduled job)

**Files:**
- Create: `functions/test/integration/dispatch_due_reminders.test.ts`

**Nota:** `dispatchDueReminders` usa `admin.messaging().send()` para enviar FCM. En el emulador de Functions, el SDK de mensajería no está emulado — las llamadas a `messaging.send()` fallarán con tokens de test. El test verifica el comportamiento de selección de tareas y deduplicación, mockeando `admin.messaging` para evitar llamadas reales a FCM.
...
const testEnv = functionsTest({ projectId: process.env.GCLOUD_PROJECT });
const wrapped = testEnv.wrap(dispatchDueReminders);
...
```bash
cd functions && npm run test:integration -- --testPathPattern=dispatch_due_reminders
```
...
```bash
git add functions/test/integration/dispatch_due_reminders.test.ts
git commit -m "test(functions): integración de dispatchDueReminders — ventana 15min, deduplicación, token ausente"
```

---

## Task 10: full_user_flow.test.ts
