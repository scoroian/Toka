...
class DartFileEditBuilderImpl extends FileEditBuilderImpl
  ...
  Future<void> importElementLibrary(Element element,
    ...
    // publiclyExporting now takes Element2 in analyzer >=7.0.0
    var libraryToImport2 = await TopLevelDeclarations(resolvedUnit)
            .publiclyExporting(element2, resultCache: null) ??
        // Fall back to the elements library if we didn't find a better one.
        element2.library2;

    var uriToImport = libraryToImport2?.uri;
    if (uriToImport != null) {
      var newImport =
          elementLibrariesToImport[element2] = _importLibrary(
        uriToImport,
      ...
      // imports.
      _removeUnnecessaryPendingElementImports(newImport, libraryToImport2 as LibraryElement?);
    }
  }
